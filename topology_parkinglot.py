...
myQueueSize = 15000  # buffer size in packets
...
def perfTest():
    ...
    net.pingAll()
    h1, h8, h3 = net.get('h1','h8', 'h3')
    h8.cmd('iperf3 -s -i 1 > h8_logs_2ii_%s &'%(myQueueSize))
    h1.cmd('iperf3 -c 10.0.0.8 -t 200 -C bbr > h1_logs_2ii_%s &'%(myQueueSize))
    h3.cmd('ping 10.0.0.7 -i 1 -c 200 > ping_2ii_%s &'%(myQueueSize))
    time.sleep(300)
